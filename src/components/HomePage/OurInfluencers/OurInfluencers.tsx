            ...
            <div className="flex items-center justify-center gap-3 mt-[56px]">
                {
                    influencerCategoryButtons.map(btn =>
                        <button key={btn} onClick={() => setActiveTab(btn)} className={`cursor-pointer hover:bg-neutral-40 transition duration-500 rounded-[100px] px-6 py-[10px] font-Satoshi text-2xl leading-7 border-2 ${activeTab === btn ? "text-primary-15 border-primary-15" : "text-neutral-50 border-neutral-50 bg-neutral-45"}`}>{btn}</button>
                    )
                }
            </div>
        </div>
